...
ManulForecast/
  ...
  WeatherAssets/              包含 USDZ 角色资源的本地 Swift Package
...
该脚本直接构建 `ManulForecast` target 并安装到模拟器，避免 Xcode 26.4 在部分环境下把 scheme destination 误判为必须安装 iOS 26.4 runtime。
...
项目包含 3D 资产。当前仓库历史已经清理过，大文件不超过 GitHub 的 100 MB 限制；目前最大的文件是 `WeatherAssets/Sources/WeatherAssets/Manul.usdz`，约 11 MB。如果后续 3D 资产继续变大，可以考虑使用 Git LFS。
...
ManulForecast/
  ...
  WeatherAssets/              Local Swift package with USDZ character assets
...
To build and launch the current code from the command line:

```bash
scripts/run-simulator.sh
```

The script builds the `ManulForecast` target directly and installs it on the simulator, avoiding an Xcode 26.4 scheme-destination issue that can incorrectly require an iOS 26.4 runtime.
...
This project includes 3D assets. The repository history has been cleaned, and no file currently exceeds GitHub's 100 MB hard limit. The largest current file is `WeatherAssets/Sources/WeatherAssets/Manul.usdz`, at about 11 MB. If future source assets grow significantly, Git LFS should be considered.
